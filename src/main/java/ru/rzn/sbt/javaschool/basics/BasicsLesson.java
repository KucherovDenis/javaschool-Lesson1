...
public class BasicsLesson {
    ...
    public Object newClass() {
        Object o = new LethalWeapon("Red", 1.0, 10);
        return o;
    }
    ...
    public int stringExercise2(String input) {
        ...
        for (char c : chars) {
            if (c == ',') countComma++;
            else if (c == '-') countHyphen++;
            else if (c == ' ') countSpace++;
        }

        int indexFirstR = input.indexOf("R");
        int indexLastL = input.lastIndexOf("L");

        return len * countComma * countHyphen * countSpace * indexFirstR * indexLastL;

        /*int count = input.length();
        count *= (int) input.chars().filter(ch -> ch == ',').count();
        count *= (int) input.chars().filter(ch -> ch == '-').count();
        count *= input.indexOf("R");
        count *= input.lastIndexOf("L");
        count *= (int) input.trim().chars().filter(ch -> ch == ' ').count();
        return count;*/
    }
    ...
    public String stringExercise3(String input) {
        ...
        for (String str : strs) {
            if ("Hello".equals(str)) isHello = true;
            if ("Goodbye".equals(str)) isGoodbye = true;

            if (isHello || isGoodbye) break;
        }

        if (strs.length == 3) {
            result = strs[0] + " " + strs[strs.length - 1];
            ...
            if (isHello) {
                ...
                result = strs[1].toLowerCase() + " " + strs[0].toLowerCase();
            }
        }
        return result;
